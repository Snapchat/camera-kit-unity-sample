 ...
 * // Instead, you should modify the "Camera Kit Unity Bridge API" script and access the functions through the imported class wrapper.
*/

/*
 * @param {RemoteApiResponse} response A raw API response from an Camera Kit Unity Bridge API
...
ApiModule.prototype.unity_send_data = function(data, cb) {
    var req = global.RemoteApiRequest.create();
    print("Sending data " + data)
    req.endpoint = "unity_send_data";
    var parameters = {};
    setParameter("unityData", data, parameters, true);
...
ApiModule.prototype.unity_request_state = function(cb) {
    var req = global.RemoteApiRequest.create();
    req.endpoint = "unity_request_state";
    this.remoteServiceModule.performApiRequest(req, function(response) {
        if(cb) {
            handleAPIResponse(response, cb);
        }
